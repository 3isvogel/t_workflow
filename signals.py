...
def pass_filter(band: list[float, float], sample_f:float, btype='band', order: int = 3) -> Callable:
    
    """Apply butter filter to a signal, for use with `Transformer`

    Args:
        band (list[float, float]) two ends of the band to use, low end is not used for low-pass, high end is not used for high-pass
